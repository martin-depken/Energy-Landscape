...
def plot_single_mut_data(data, data_colname, Mut_type, Canonical=True, data_name='Data', Plot=True, logplot=False,
    ...
    single_mut_data_mean = single_mut_data[['Position', data_colname]].groupby('Position').median().reset_index()

    if Plot:
        plt.figure()
        if logplot:
            plt.semilogy(single_mut_data_mean['Position'], single_mut_data_mean[data_colname]
                         , marker='o', linewidth=1, color='blue')
        else:
            plt.plot(single_mut_data_mean['Position'], single_mut_data_mean[data_colname]
                         , marker='o', linewidth=1, color='blue')
...
def plot_double_mut_data(data, data_colname, Mut_type, Canonical=True, Ng=20, data_name='Data', Plot=True, logplot=False,
                        SaveFigures=False, figure_name='./Figure.pdf'):
    Mut_names = {'r': 'Mismatch', 'i': 'Insertion', 'd': 'Deletion'}
    Mut_name = Mut_names[Mut_type]
    ...
    double_mut_data_mean = double_mut_data.groupby('Mutation Positions').median().reset_index()
    double_mut_map = np.empty((Ng, Ng))
    double_mut_map[:] = np.nan
    for n in range(len(double_mut_data_mean)):
        pos = double_mut_data_mean['Mutation Positions'].iloc[n]
        Positions = map(lambda x: int(x) - 1, pos.split('|'))
        datapoint = double_mut_data_mean[data_colname].iloc[n]
        if logplot:
            datapoint = np.log(datapoint)
    ...
    return double_mut_data_mean, double_mut_df
