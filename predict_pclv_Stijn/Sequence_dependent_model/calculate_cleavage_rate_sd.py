...
def get_master_equation(parameters, xdata, model_id, guide_length):
    ...
    energies = get_energies(epsilon_si,epsilon_i_sd,xdata, guide_length)
    backward_rates = get_backward_rates(energies, rates_si,guide_length )
    rate_matrix = build_rate_matrix(rates_si, backward_rates)
    return rate_matrix

def get_energies(epsilon_si,epsilon_i_sd,xdata, guide_length=20):
    '''
...
def calc_ABA(parameters, concentrations, reference, xdata, model_id = 'general_energies', guide_length = 20, T=10*60):
    Kd, _,_ = calc_Pbound(parameters, concentrations, reference, xdata, model_id, guide_length, T)
    return np.log(Kd)

def calc_Pbound(parameters, concentrations, reference, xdata, model_id = 'general_energies', guide_length = 20, T=10*60):
    #print('at the beginning', concentrations)
    rate_matrix = get_master_equation(parameters, xdata, model_id, guide_length)
